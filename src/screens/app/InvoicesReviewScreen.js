...
function getStatusStyle(inv) {
  if (inv.canceled) return { bg: '#fee2e2', border: '#fca5a5', text: '#dc2626', label: 'Canceled' };
  const paid = sumPayments(inv.payments);
  const balance = (parseFloat(inv.totalAmount) || 0) - paid;
  if (balance <= 0.01) return { bg: '#dcfce7', border: '#86efac', text: '#16a34a', label: 'Paid' };
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const fmt = (n) => formatCurrency(n, item._currency);
          return (
            <View style={styles.card}>
              ...
              <View style={[styles.finRow, styles.finRowBalance]}>
                ...
                <Text style={[styles.finValueBold, { color: item._balanceDue > 0.01 ? '#dc2626' : '#16a34a' }]}>
              ...
              {item.poSupplier?.order ? (
                <View style={styles.poRow}>
                  <Ionicons name="link-outline" size={12} color="#9fb8d4" />
...
const styles = StyleSheet.create({
  ...
  invNum: { fontSize: 14, fontWeight: '700', color: '#0366ae' },
  clientName: { fontSize: 12, color: '#103a7a', marginTop: 2 },
  dateText: { fontSize: 11, color: '#9fb8d4', marginTop: 2 },
  statusBadge: {
    paddingHorizontal: 10, paddingVertical: 4,
    borderRadius: 999, borderWidth: 1,
  },
  statusText: { fontSize: 11, fontWeight: '700' },
  divider: { height: 1, backgroundColor: '#f0f8ff', marginBottom: 8 },
  ...
  finLabel: { fontSize: 11, color: '#9fb8d4', fontWeight: '600' },
  finValue: { fontSize: 12, color: '#103a7a', fontWeight: '600' },
  finLabelBold: { fontSize: 12, color: '#103a7a', fontWeight: '700' },
  ...
  poText: { fontSize: 11, color: '#9fb8d4' },
