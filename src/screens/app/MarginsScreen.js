...
// Normalize date — web stores it as { startDate, endDate } object
...
// Max 3 decimal places validation
...
// Auto-calc item fields from purchase/shipped/margin
const recalcItem = (item) => {
  ...
  const totalMargin = purchase * margin;
  const remaining = openShip * margin;
  return {
    ...item,
...
// Aggregate month-level totals from items (gis items count half for remaining/totalMargin)
const recalcMonthTotals = (items) => {
  const purchase = items.reduce((s, x) => s + (parseFloat(x.purchase) || 0), 0);
  const openShip = items.reduce((s, x) => s + (parseFloat(x.openShip) || 0), 0);
  const remaining = items.reduce((s, x) => s + (x.gis ? (parseFloat(x.remaining) || 0) / 2 : (parseFloat(x.remaining) || 0)), 0);
  const totalMargin = items.reduce((s, x) => s + (x.gis ? (parseFloat(x.totalMargin) || 0) / 2 : (parseFloat(x.totalMargin) || 0)), 0);
  return { purchase, openShip, remaining, totalMargin };
};
...
export default function MarginsScreen({ navigation }) {
  ...
  // ─── Overall totals (non-halved, straight sum of month-level aggregates) ──
  ...
  // ─── GIS-only data per month ───────────────────────────────────────────────
  const gisMonthsData = useMemo(() => monthsData.map(m => {
    const gisItems = (m.items || []).filter(x => x.gis);
    return {
      month: m.month,
      purchase: gisItems.reduce((s, x) => s + (parseFloat(x.purchase) || 0), 0),
      openShip: gisItems.reduce((s, x) => s + (parseFloat(x.openShip) || 0), 0),
      remaining: gisItems.reduce((s, x) => s + (parseFloat(x.remaining) || 0), 0),
      totalMargin: gisItems.reduce((s, x) => s + (parseFloat(x.totalMargin) || 0), 0),
    };
  }), [monthsData]);

  const gisTotals = gisMonthsData.reduce((acc, m) => {
    acc.purchase += m.purchase;
    acc.openShip += m.openShip;
    acc.remaining += m.remaining;
    acc.totalMargin += m.totalMargin;
    return acc;
  }, { purchase: 0, openShip: 0, remaining: 0, totalMargin: 0 });
    ...
    Alert.alert('Delete Month', `Remove month ${MONTH_NAMES[parseInt(month, 10) - 1]}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          ...
          try {
            const docRef = doc(db, uidCollection, 'margins', String(year), String(month));
  ...
  // ─── Update edit item field with auto-calc ────────────────────────────────
  const handleEditChange = (field, value) => {
    // Decimal validation for numeric fields
    if (field !== 'description' && field !== 'date' && field !== 'supplier' && field !== 'client') {
      if (countDecimals(value) > 3) return;
    }
    setEditItem(prev => {
      const updated = { ...prev, [field]: value };
      if (['purchase', 'shipped', 'margin'].includes(field)) {
  ...
  const handleSaveItem = async () => {
    ...
    const updatedMonths = monthsData.map(m => {
      ...
      const existsIdx = m.items.findIndex(x => x.id === finalItem.id);
      const newItems = existsIdx >= 0
        ? m.items.map((x, i) => i === existsIdx ? finalItem : x)
        : [...m.items, finalItem];
      ...
      <View style={[styles.root, { paddingTop: insets.top }]}>
        ...
        >
          {monthsData.map(m => {
            ...
            const mMargin = parseFloat(m.totalMargin) || 0;
                ...
                <View style={styles.monthHeaderWrap}>
                  ...
                  >
                    ...
                    <View style={styles.monthHeaderRight}>
                      ...
                      <Text style={styles.monthMarginVal}>{fmtUSD(mMargin)}</Text>
                              ...
                              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                                ...
                                {item.gis && (
                                  <View style={styles.gisBadge}>
                                    <Text style={styles.gisBadgeText}>GIS</Text>
                                  </View>
                                )}
                              </View>
          ...
          {/* ─── ThirdPart: GIS-only per month ──────────────────────────── */}
          {gisMonthsData.some(m => m.purchase > 0 || m.totalMargin > 0) && (
            <ThirdPartTable
              title="Total GIS"
              rows={gisMonthsData}
              grandTotals={gisTotals}
              year={year}
            />
          )}
        </ScrollView>
...
// ─── ThirdPart per-month breakdown table ──────────────────────────────────────
        ...
        return (
          <View key={i} style={[styles.tpRow, i % 2 === 1 && styles.tpRowAlt]}>
            <View style={[styles.tpMonthCell, { flex: 1.2 }]}>
              <Text style={styles.tpMonthText}>{monthName}-{year}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  gisBadge: {
    backgroundColor: C.warningDim, borderRadius: 4,
  ...
  tpTitle: { fontSize: 13, fontWeight: '700', color: C.text1, padding: 12, paddingBottom: 8 },
  tpRow: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: C.border },
  tpRowAlt: { backgroundColor: C.bgSecondary },
  tpCell: { fontSize: 10, color: C.text1, paddingHorizontal: 6, paddingVertical: 7, textAlign: 'center' },
  tpHeader: { backgroundColor: C.bgTertiary, fontWeight: '700', color: C.accent, paddingVertical: 8 },
  tpMonthCell: { justifyContent: 'center', alignItems: 'center', paddingHorizontal: 6, paddingVertical: 6 },
  tpMonthText: {
