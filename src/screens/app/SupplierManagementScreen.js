...
const EMPTY = {
  supplier: '', nname: '', street: '', city: '', country: '',
  other1: '', poc: '', email: '', phone: '', mobile: '', fax: '', other2: '',
  deleted: false,
};

const REQUIRED = ['supplier', 'nname', 'street', 'city', 'country'];
...
export default function SupplierManagementScreen({ navigation }) {
  ...
  const allSuppliers = settings?.Supplier?.Supplier || [];
  const active = allSuppliers
    .filter(x => !x.deleted)
    .sort((a, b) => (a.supplier || '').localeCompare(b.supplier || ''));

  const filtered = search.trim()
    ? active.filter(x => x.supplier.toLowerCase().includes(search.toLowerCase()) || (x.nname || '').toLowerCase().includes(search.toLowerCase()))
    : active;
  ...
  const persistAndSync = async (newArr) => {
    ...
    await saveDataSettings(uidCollection, SETTINGS_DOCS.SUPPLIER, newObj);
    setSettings(prev => ({ ...prev, Supplier: newObj }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      let newArr;
      if (form.id) {
        newArr = allSuppliers.map(x => x.id === form.id ? { ...form } : x);
      } else {
        newArr = [...allSuppliers, { ...form, id: uuidv4() }];
      }
  ...
  const handleDelete = () => {
    Alert.alert('Delete Supplier', `Remove "${form.supplier}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          const newArr = allSuppliers.map(x => x.id === form.id ? { ...x, deleted: true } : x);
          await persistAndSync(newArr);
          setModalVisible(false);
          setForm(EMPTY);
        },
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <AppHeader title="Suppliers" navigation={navigation} showBack />
      ...
      <Text style={styles.count}>{active.length} suppliers</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No suppliers yet. Tap + to add one.</Text>}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onPress={() => openEdit(item)} activeOpacity={0.7}>
            <View style={styles.cardIconWrap}>
              <Ionicons name="person-outline" size={20} color="#0366ae" />
            </View>
            <View style={styles.cardBody}>
              <Text style={styles.cardName}>{item.supplier}</Text>
      ...
      <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={[styles.modalRoot, { paddingTop: insets.top }]}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{form.id ? 'Edit Supplier' : 'Add Supplier'}</Text>
          ...
          <ScrollView contentContainerStyle={styles.modalScroll} keyboardShouldPersistTaps="handled">
            ...
            <Field label="Name" field="supplier" required />
          ...
          <View style={[styles.modalFooter, { paddingBottom: insets.bottom + 12 }]}>
            ...
            <TouchableOpacity
              ...
              disabled={saving}
            >
              <Text style={styles.saveBtnText}>{saving ? 'Saving...' : form.id ? 'Update' : 'Add Supplier'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
